...
class PageTypeResolverService @Inject()(
  ...
  def resolve(pageType: PageType)(implicit request: ServiceInfoRequest[AnyContent], messages: Messages): Html = pageType match {
    ...
    case p@PageType.ViewOrCorrectYourSubmissions => epaye_view_or_correct_submissions(p.name, request.request.email, appConfig)(messages)
    case p@PageType.PayeCisRefunds => paye_cis_refunds(p.name)(messages)
    case p@PageType.PayeChangeCircumstance => paye_change_circumstance(p.name, appConfig)(messages)
    case p@PageType.PayeStopEmployer => paye_stop_employer(p.name, appConfig)(messages)
    case p@PageType.RegisterAddCT => ctax_register_add(p.name, appConfig)(messages)
